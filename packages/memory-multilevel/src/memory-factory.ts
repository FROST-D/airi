...
// Type imports only (no runtime imports) for optional providers
import type { PostgresPgvectorMemoryOptions } from './providers/long-term/postgres-pgvector.provider'
import type { LocalRedisShortTermMemoryOptions } from './providers/short-term/local-redis.provider'
...
import { PostgresPgvectorMemoryProvider } from './providers/long-term/postgres-pgvector.provider'
import { LocalRedisShortTermMemoryProvider } from './providers/short-term/local-redis.provider'

// Optional providers - these will throw errors if used but dependencies aren't installed
type QdrantMemoryOptions = any
type UpstashRedisShortTermMemoryOptions = any
type VercelKvShortTermMemoryOptions = any
...
export class MemorySystemFactory {
  static async create(options: MemorySystemFactoryOptions = {}): Promise<CombinedMemoryProvider> {
  ...
  private static resolveLongTermProvider(envVars: NodeJS.ProcessEnv, options: MemorySystemFactoryOptions): IMemoryProvider | null {
    ...
    if (providerName === 'qdrant') {
      throw new Error('Qdrant provider not available. Install @qdrant/js-client-rest and @xsai/embed to use this provider.')
    }
